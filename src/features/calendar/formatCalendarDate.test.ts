...
describe("formatFriendlyLongDate", () => {
  it("formats English ordinal dates", () => {
    expect(formatFriendlyLongDate(new Date(2026, 8, 13), "en-US")).toBe(
      "13th September, 2026",
    );
  ...
  it("formats Spanish long dates", () => {
    expect(formatFriendlyLongDate(new Date(2026, 8, 13), "es-ES")).toBe(
...
describe("isSameCalendarDay", () => {
  it("compares calendar days ignoring time", () => {
    expect(
      isSameCalendarDay(new Date(2026, 8, 4, 9, 0), new Date(2026, 8, 4, 23, 0)),
    ).toBe(true);
